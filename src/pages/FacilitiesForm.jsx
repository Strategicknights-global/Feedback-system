            ...
            <div className="space-y-10">
              {questions.map(({ question, originalIndex }) => (
                <div key={originalIndex}>
                  ...
                  {/* --- RATING SECTION WITH NEW ICONS --- */}
                  <div className="flex items-center justify-center space-x-6 sm:space-x-8">
                    {RATING_VALUES.map(value => (
                      <div key={value} className="text-center">
                        <label className="group flex flex-col items-center cursor-pointer space-y-2">
                          <input
                            type="radio"
                            ...
                            required
                          />
                          {/* Replace the old div with the new RatingIcon component */}
                          <RatingIcon
                            rating={value}
                            isSelected={ratings[originalIndex] === value}
                          />
                          {/* Style the number below to match the selection state */}
                          <span className={`text-sm font-semibold transition-colors ${
                              ratings[originalIndex] === value ? 'text-indigo-700' : 'text-slate-500'
                          }`}>
                            {value}
                          </span>
                        </label>
